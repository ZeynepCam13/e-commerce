  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", p: { xs: 2, md: 4 } }}>
      ...
      <Box sx={{ mt: 10, maxWidth: 800 }}>
        <Typography variant="h5" fontWeight="700" mb={4}>Müşteri Yorumları ({comments.length})</Typography>
        
        {user && (
          <Paper elevation={0} sx={{ p: 3, border: "1px solid #e0e0e0", mb: 4, borderRadius: 2 }}>
            <Typography fontWeight="bold" mb={2}>Deneyiminizi paylaşın</Typography>
            <TextField
              fullWidth
              multiline
              rows={3}
              ...
              sx={{ mb: 2 }}
            />
            <Button variant="contained" onClick={handleAddComment} sx={{ bgcolor: "black" }}>Yorum Yap</Button>
          </Paper>
        )}

        <Stack spacing={3}>
          {comments.map((c) => (
            <Box key={c.id} sx={{ pb: 3, borderBottom: "1px solid #f0f0f0" }}>
              <Stack direction="row" spacing={2} alignItems="center" mb={1}>
                <Box sx={{ width: 40, height: 40, bgcolor: "grey.200", borderRadius: "50%", display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                  {c.username[0].toUpperCase()}
                </Box>
                <Box>
                  <Typography fontWeight="bold">{c.username}</Typography>
                  <Typography variant="caption" color="text.secondary">24 Aralık 2025</Typography>
                </Box>
              </Stack>
