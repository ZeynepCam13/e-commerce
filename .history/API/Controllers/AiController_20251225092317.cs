...
else 
{
    ...
    var searchWord = cleanedMessage.ToLower();
    q = q.Where(p => 
        p.Name.ToLower().Contains(searchWord) || 
        (p.Description != null && p.Description.ToLower().Contains(searchWord)) ||
        (p.subSubCategory != null && p.subSubCategory.Name.ToLower().Contains(searchWord))
    );
}

var products = await q.Take(5).Select(p => new { p.Id, p.Name, p.Price }).ToListAsync();
       ...
       var systemPrompt = """
Sen neşeli ve profesyonel bir e-ticaret satış danışmanısın. 
GÖREVİN: Kullanıcının mesajına göre elindeki ürün listesini analiz edip en uygun ürünleri önermek.
KURAL 1: Eğer ürün listesi doluysa mutlaka o ürünleri isim, fiyat ve link vererek öner. ASLA "yok" deme.
KURAL 2: Linkleri SADECE sana verilen "/catalog/ID" formatında yaz. Başına "https://example.com" ekleme!
KURAL 3: Eğer ürün listesi tamamen boşsa, o zaman kullanıcıdan daha spesifik bir arama yapmasını iste veya popüler kategorileri öner.
""";
