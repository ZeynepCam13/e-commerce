...
export default function ProductDetails() {
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", p: { xs: 2, md: 4 } }}>
      ...
      {/* YORUMLAR SEKSİYONU - Güncellenmiş Güvenli Bölüm */}
<Stack spacing={3}>
  {comments && comments.map((c) => (
    <Box key={c.id} sx={{ pb: 3, borderBottom: "1px solid #f0f0f0" }}>
      <Stack direction="row" spacing={2} alignItems="center" mb={1}>
        {/* Avatar kısmını güvenli hale getirdik */}
        <Box sx={{ 
          width: 40, height: 40, bgcolor: "grey.200", borderRadius: "50%", 
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' 
        }}>
          {/* Username varsa ilk harfini al, yoksa '?' koy */}
          {c.username ? c.username[0].toUpperCase() : '?'}
        </Box>
        <Box>
          <Typography fontWeight="bold">
            {c.username || "Anonim Kullanıcı"}
          </Typography>
          <Typography variant="caption" color="text.secondary">
            {/* Tarih verisi backend'den geliyorsa c.date ekleyin */}
            24 Aralık 2025
          </Typography>
        </Box>
      </Stack>
