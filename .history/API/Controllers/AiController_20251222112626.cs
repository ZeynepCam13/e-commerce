...
public class AiController : ControllerBase
{
    ...
    public async Task<IActionResult> Chat([FromBody] AiChatRequestDto dto)
    {
        ...
        var productText = products.Any()
            ? string.Join("\n", products.Select((p, i) =>
                $"{i + 1}. {p.Name} - {p.Price} TL "+ $"Ürün Linki:{baseUrl} /catalog/{p.Id}\n"
              ))
...
You MUST output PRODUCT_LINK exactly as provided.
Never rewrite, shorten, expand, or invent URLs.
Never replace domains.
...
public async Task<IActionResult> GetSimilarProducts(
    [FromBody] AiSimilarProductsRequestDto dto)
{
    ...
    // Aynı kategorideki diğer ürünleri al
    ...
    // ŞİMDİLİK AI YOK → basit benzerlik
    return Ok(candidates.Take(4));
}
