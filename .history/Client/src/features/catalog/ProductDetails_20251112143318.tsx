...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    if (product) {
      ...
      // Ana resim: varsayılan olarak product.imageUrl veya ilk galeri resmi
      if (product.images && product.images.length > 0) {
        setMainImage(`http://localhost:5198/${product.images[0].imageUrl}`);
      } else {
        setMainImage(`http://localhost:5198/${product.imageUrl}`);
      }
    }
  }, [product, dispatch]);
  ...
  const handleAddComment = async () => {
    ...
    await dispatch(addComment({ productId: Number(product.id), text: commentText }));
    setCommentText("");
  };
  ...
  return (
    <Stack direction={{ xs: "column", md: "row" }} spacing={6}>
      {/* Ana ürün görseli */}
      <Box
        sx={{ width: { xs: "100%", md: "45%", lg: "33%" }, position: "relative" }}
      >
        {mainImage && (
          <img
            src={mainImage}
          ...
          >
            {product.images.map((img, i) => (
              <img
                ...
                width={70}
                height={70}
                onClick={() => setMainImage(`http://localhost:5198/${img.imageUrl}`)}
                style={{
                  borderRadius: "6px",
                  cursor: "pointer",
                  border:
                    mainImage === `http://localhost:5198/${img.imageUrl}`
                      ? "2px solid black"
                      : "1px solid #ccc",
                  objectFit: "cover",
                }}
