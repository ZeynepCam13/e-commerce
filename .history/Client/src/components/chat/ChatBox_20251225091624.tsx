...
function renderMessage(text: string) {
  ...
  return parts.map((part, index) => {
    if (/\/catalog\/\d+/.test(part)) {
      ...
      const cleanPath = part.trim().replace(/[()\[\]]/g, ""); 
      ...
      return (
        <Button
          ...
          variant="contained"
          sx={{
            mt: 1, mb: 1, bgcolor: "white", color: "black",
            fontWeight: "bold", textTransform: "none", display: "block",
    ...
    const cleanText = part.replace(/https?:\/\/example\.com[^\s)]*/g, "").trim();
