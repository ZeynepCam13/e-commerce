...
export default function SettingsPage() {
  ...
  const changeEmail = async () => {
    try {
      await axios.post("http://localhost:5198/api/account/change-email", {
        newEmail,
      });
      toast.success("E-posta doğrulama linki gönderildi!");
    } catch {
      toast.error("Mail güncellenemedi!");
    }
  };
  ...
  return (
    <Container maxWidth="sm" sx={{ mt: 5 }}>
      ...
      {/* E-POSTA */}
      <Typography variant="h6">E-posta Değiştir</Typography>
      <TextField label="Yeni E-posta" fullWidth sx={{ mt: 2 }} value={newEmail} onChange={(e) => setNewEmail(e.target.value)} />
      <Button variant="contained" sx={{ mt: 2 }} onClick={changeEmail}>
        Mail Gönder
      </Button>
