...
public async Task<IActionResult> AnalyzeProductImage(IFormFile file)
{
    ...
    // 1. Resmi Base64 formatına dönüştür
    using var ms = new MemoryStream();
    await file.CopyToAsync(ms);
    byte[] imageBytes = ms.ToArray();
    string base64Image = Convert.ToBase64String(imageBytes);

    // 2. OpenAI Vision API isteği hazırlığı
    var requestBody = new
    {
        model = "gpt-4o", // Görsel analiz için en iyi model
        messages = new[]
        {
            new
            {
                ...
                content = new object[]
                {
                    new { type = "text", text = "Bu bir e-ticaret ürünüdür. Görseli analiz et ve şu JSON formatında yanıt ver: " +
                        "{ \"name\": \"Ürün Adı\", \"description\": \"Detaylı açıklama\", \"price\": 1250, \"categoryName\": \"Kategori\" }. " +
    ...
    if (!response.IsSuccessStatusCode) return BadRequest("Görsel analiz edilemedi.");

    var result = await response.Content.ReadFromJsonAsync<JsonElement>();
    var aiResponse = result.GetProperty("choices")[0].GetProperty("message").GetProperty("content").GetString();

    return Ok(aiResponse); // Frontend bu JSON'ı alıp form alanlarına dolduracak
}
