...
export default function CatalogPage() {
  ...
  // 2. Filtreleme Mantığı (Geliştirilmiş)
  useEffect(() => {
    let result = [...products];
    ...
    // Cinsiyet/Kategori Filtrelemesi (Tüm katmanlara bakar)
    if (selectedGenders.length > 0) {
      result = result.filter(p => {
        const lowerGenders = selectedGenders.map(g => g.toLowerCase());
        
        // Ana kategori, Alt kategori veya En alt kategori isimlerinden biri eşleşiyor mu?
        const categoryMatch = p.category?.name && lowerGenders.includes(p.category.name.toLowerCase());
  ...
  // Markaları alfabetik olarak sıraladık
