...
export default function ChatBox() {
  ...
  const chatStorageKey = user
    ? `chat_messages_user_${user.id}`
    : "chat_messages_guest";

  // ⏮ önceki kullanıcıyı tut
  ...
  // 🔥 KULLANICI DEĞİŞİMİ / LOGOUT YAKALAMA
  useEffect(() => {
    // logout olduysa → eski user chat’ini sil
    if (!user && prevUserId.current !== null) {
      localStorage.removeItem(`chat_messages_user_${prevUserId.current}`);
    }
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
        ...
        <Button size="small" onClick={() => setIsOpen(false)} sx={{ minWidth: "auto", fontSize: 18 }}>
          ×
        </Button>
      </Box>
