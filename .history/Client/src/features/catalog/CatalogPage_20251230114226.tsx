...
export default function CatalogPage() {
  ...
  // 1. Veriyi API'den Çekme (Eski mantığını buraya taşıdık)
  useEffect(() => {
    setLoading(true);
    ...
    getProducts()
      .then((data: IProduct[]) => {
        ...
        // Arama sorgusu varsa (Search parametresi için ek filtre)
        if (search) {
  ...
  }, [categoryId, subCategoryId, subSubCategoryId, search]); // Parametreler değişince tetiklenir

  // 2. Yan Menü Filtreleme Mantığı
  useEffect(() => {
    let result = [...products];
