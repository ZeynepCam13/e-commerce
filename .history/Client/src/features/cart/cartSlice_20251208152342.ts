...
{ productId: number, quantity?: number;size:string}>(
    ...
    async ({productId, quantity=1,size}) => {
        try 
        {
            return await requests.Cart.addItem(productId, quantity,size);
        }
        catch(error)
        {
            console.log(error);
...
export const deleteItemFromCart = createAsyncThunk<
  Cart,
  { productId: number; quantity?: number; size:string}
>("cart/deleteItemFromCart", async ({ productId, quantity = 1,size}) => {
  return await requests.Cart.deleteItem(productId, quantity,size);
});
...
export const cartSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(deleteItemFromCart.pending, (state, action) => {
            console.log(action);
            state.status = "pendingDeleteItem"+ action.meta.arg.productId+action.meta.arg.size ;
        });
