...
export default function ChatBox() {
  ...
  // 🔑 token bazlı storage key (EN GÜVENİLİR)
  const chatStorageKey = user?.token
    ? `chat_messages_user_${user.token}`
    : "chat_messages_guest";

  // 🔁 önceki token
  const prevToken = useRef<string | null>(user?.token ?? null);
  ...
  // 🔥 LOGOUT YAKALAMA (TOKEN GİTTİYSE)
  useEffect(() => {
    if (!user?.token && prevToken.current) {
      // tüm user chatlerini sil
      Object.keys(localStorage)
        .filter((k) => k.startsWith("chat_messages_user_"))
    ...
    prevToken.current = user?.token ?? null;
  }, [user?.token]);
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
        ...
        <Button size="small" onClick={() => setIsOpen(false)} sx={{ fontSize: 18 }}>
          ×
        </Button>
      </Box>
      ...
      <Box sx={{ height: 220, overflowY: "auto", mb: 1 }}>
        ...
        {loading && <Typography fontSize="0.8rem">Yazıyor...</Typography>}
      </Box>
