...
// Backend’ten gelen string → number'a dönüştürme
...
// Dropdown değerlerinin backend’teki enum karşılıkları
...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    if (selectedOrder) {
      const stringStatus = selectedOrder.orderStatus; // "Pending"
        ...
        <TableBody>
          {orders.map((o) => (
            <TableRow key={o.id}>
              ...
              {/* backend'ten gelen string gösterilir */}
              <TableCell>{String(o.orderStatus)}</TableCell>
        ...
        <DialogContent dividers>
          {selectedOrder && (
            <>
              ...
              <Typography variant="subtitle1" gutterBottom>
                <strong>Durum:</strong> {String(selectedOrder.orderStatus)}
              </Typography>
              <Typography variant="subtitle1" gutterBottom>
