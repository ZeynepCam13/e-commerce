...
export default function CatalogPage() {
  ...
  // Filtre State'leri (Tipler string[] olarak güncellendi)
  ...
  // 1. Veriyi API'den Çekme (Kategoriler dahil)
  useEffect(() => {
    setLoading(true);
    ...
    getProducts()
      .then((data: IProduct[]) => {
        ...
        // URL'den gelen bir search (arama) varsa filtrele
        if (search) {
  ...
  // 2. Sol Panel Filtreleme Mantığı
  useEffect(() => {
    let result = [...products];
    ...
    // Cinsiyet/Kategori Filtresi (Tüm katmanlara bakan gelişmiş mantık)
  ...
  // Markaları alfabetik olarak sıralama (A-Z)
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Grid container spacing={3}>
        ...
        {/* SAĞ PANEL - Ürün Listesi Bölümü */}
        <Grid item xs={12} md={9} lg={9.5}>
          {loading ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 10 }}>
                <Typography>Ürünler Hazırlanıyor...</Typography>
            </Box>
          ) : (
