...
const Catalog = {
  ...
  similarProducts: (id: number) =>
    queries.get(`products/${id}/similar`),
  ...
  bySubCategory: (subCategoryId: number) =>
    queries.get(`products/bySubCategory/${subCategoryId}`),


  bySubSub: (subSubCategoryId: number) =>
    queries.get(`products/bySubSub/${subSubCategoryId}`),
...
const Cart = {
  ...
  addItem: (productId: number, quantity = 1,size:string) =>
    queries.post(`cart?productId=${productId}&quantity=${quantity}&size=${size}`,{}),
  deleteItem: (productId: number, quantity = 1,size:string) =>
    queries.delete(`cart?productId=${productId}&quantity=${quantity}&size=${size}`),
};
...
const Category = {
  list: () => queries.get("category/tree"),
  tree: () => queries.get("category/tree"), 
...
const AdminAnalytics = {
  summary: () => queries.get("/admin/analytics/summary"),
  monthlyRevenue: () => queries.get("/admin/analytics/monthly-revenue"),
  topProducts:()=>queries.get("/admin/analytics/top-products"),
  orderStatus: () => queries.get("/admin/analytics/order-status"),
  dailyRevenue:()=>queries.get("admin/analytics/day-revenue"),
  weeklyRevenue: () => queries.get('admin/analytics/weekly-revenue')
