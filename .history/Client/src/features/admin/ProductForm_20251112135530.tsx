...
interface Props {
  product: any;            // düzenleme modunda gelen ürün
  cancelEdit: () => void;  // iptal butonu için
}

type Category = { id: number; name: string };

export default function ProductForm({ product, cancelEdit }: Props) {
  // Form state (ürün ekle/düzenle)
  ...
  // Çoklu galeri dosyaları + önizleme
  ...
  // product prop'u değişirse formu sync et
  useEffect(() => {
    setFormData({
      id: product?.id ?? 0,
      name: product?.name ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Kapak görseli seçildiyse önce yükle, imageUrl'ı güncelle
    if (file) {
      try {
        const res = await requests.Admin.uploadImage(file);
        setFormData((prev: any) => ({ ...prev, imageUrl: res.data.imageUrl }));
        // requests ile devam ederek imageUrl formData içinde hazırlanmış olacak
