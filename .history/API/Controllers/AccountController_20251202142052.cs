...
public async Task<IActionResult> ForgotPassword(ForgotPasswordDTO model)
{
    var user = await _userManager.FindByEmailAsync(model.Email);
    if (user == null)
        return Ok(); // Email vermeyiz (güvenlik için)
...
public async Task<IActionResult> ChangeEmail(ChangeEmailDTO dto)
{
    ...
    var url = $"http://localhost:3000/verifyemail?email={dto.NewEmail}&token={Uri.EscapeDataString(token)}";

    await _emailService.SendEmailAsync(dto.NewEmail, "E-posta Doğrulama", 
        $"Yeni e-posta adresinizi doğrulamak için tıklayın: <a href='{url}'>E-postayı Doğrula</a>");
