...
function renderMessage(text: string) {
  // 1. Botun gönderdiği (https://example.com/catalog/123) veya (/catalog/123) 
  // gibi yapıları sadece /catalog/123 kalacak şekilde temizliyoruz.
  const sanitizedText = text
    .replace(/https?:\/\/[^\s)]+/g, "") // Dış linkleri sil
    .replace(/[()\[\]]/g, " "); // Parantezleri boşluğa çevir

  // 2. Metni /catalog/ID kalıbına göre bölüyoruz
  const parts = sanitizedText.split(/(\/catalog\/\d+)/g);

  return parts.map((part, index) => {
    // Eğer parça bir ürün yoluysa (/catalog/7001 gibi)
    if (/\/catalog\/\d+/.test(part)) {
      ...
      return (
        <Button
          key={index}
          component={Link}
          to={cleanPath} // Artık burası kesinlikle "/catalog/7011" formatında
          variant="contained"
          sx={{
            mt: 1, mb: 1.5,
            bgcolor: "white", // Eski çalışan kodundaki gibi açık renk veya siyah tercih edebilirsin
            color: "black",
            fontWeight: "bold",
            textTransform: "none",
            display: "block",
