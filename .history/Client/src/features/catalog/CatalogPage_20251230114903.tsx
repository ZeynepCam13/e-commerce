...
export default function CatalogPage() {
  ...
  // Markaları alfebetik sıralıyoruz
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Grid container spacing={3}>
        {/* SOL PANEL: Loading'den bağımsız olarak render edilir */}
        <Grid item xs={12} md={3} lg={2.5}>
          <Paper sx={{ p: 2, borderRadius: 0, border: "1px solid #eee", boxShadow: "none" }}>
            ...
            <FormGroup>
              {/* Ürünler yüklenene kadar boş kalmasın diye fallback ekledik */}
              {brands.length > 0 ? (
                brands.map((brand) => (
                  <FormControlLabel
                    key={brand}
        ...
        {/* SAĞ PANEL: Sadece burası loading durumuna göre değişir */}
        <Grid item xs={12} md={9} lg={9.5}>
          {loading ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
              <CircularProgress color="inherit" />
              <Typography sx={{ ml: 2 }}>Ürünler yükleniyor...</Typography>
            </Box>
          ) : (
