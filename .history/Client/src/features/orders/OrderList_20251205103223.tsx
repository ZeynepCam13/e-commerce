...
export default function OrderList()
{
    ...
    useEffect(() => {
        ...
        .then(orders => {console.log("HER ORDER:", orders);
  ...
  {orders?.map((order) => {
    ...
    console.log("GERÇEK ORDER", order);   // 🔥 BURAYA
    
    return (
      <TableRow key={order.id}>
        <TableCell component="th" scope="row">{order.id}</TableCell>
        <TableCell component="th" scope="row">
          {statusToText[Number(order.orderStatus)] ?? order.orderStatus}
        </TableCell>
                            ...
                            <TableBody>
                                {selectedOrder?.orderItems.map((item) => (
                                    <TableRow key={item.id}>
                                        <TableCell>
                                            <img src={`http://localhost:5198/${item.productImage}`} style={{height: 60}}/>
                                        </TableCell>
                                        <TableCell align="right">{item.productName}</TableCell>
                                        <TableCell align="right">{item.sizes}</TableCell>
