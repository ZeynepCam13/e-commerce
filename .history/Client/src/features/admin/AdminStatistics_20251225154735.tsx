...
export default function AdminStatistics() {
  ...
  useEffect(() => {
    // Backend'deki GetStoreStats metoduna istek atıyoruz
    axios.get("/ai/stats") // Controller route [Route("api/ai")] ise "/ai/stats" kullanın
      .then(res => {
        setStats(res.data);
        setLoading(false);
      })
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: "#fcfcfc", minHeight: "100vh" }}>
      <Container maxWidth="lg">
        ...
        {/* Özet Kartları - Backend'den Gelen Gerçek Veriler */}
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <StatCard 
            title="Büyüme" 
            value="+%0" // Burası backend'de hesaplanmadığı için statik veya 0 kalsın
            icon={<TrendingUp />} 
          />
        </Grid>

        <Grid container spacing={3}>
          {/* Satış Trendi Grafiği (LineChart) */}
          ...
          {/* Kategori Bazlı Dağılım (BarChart) */}
