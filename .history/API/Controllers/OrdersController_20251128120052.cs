...
namespace API.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        public OrdersController(DataContext context, IPublishEndpoint publishEndpoint)
        {
            _context = context;
...

        {
            ...
            if (result)
            {
                ...
                await _publishEndpoint.Publish(new InvoiceMessage
                {
                    OrderId = order.Id,
                    Email = user!.Email!,
    ...
    // İleri yönlü kontrol
            if ((int)dto.Status <= (int)order.OrderStatus)
    ...
    // RabbitMQ mesajı gönder
            await _publishEndpoint.Publish(new OrderStatusChangedMessage
           {
            OrderId = order.Id,
            Email = user?.Email ?? "",
