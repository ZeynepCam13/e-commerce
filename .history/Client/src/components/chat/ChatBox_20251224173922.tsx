...
function renderMessage(text: string) {
  // Hem linki yakalar hem de etrafındaki "Link:" gibi ifadeleri temizlemeye yardımcı olur
  ...
  return parts.map((part, index) => {
    if (/\/catalog\/\d+/.test(part)) {
      return (
        <Button
          key={index}
          component={Link}
          to={part.trim()} // Varsa gizli boşlukları temizler
          variant="contained"
          size="small"
          sx={{
            mt: 1, mb: 1,
    ...
    // "Link:" veya "Ürün Linki:" yazılarını botun cevabından temizlemek istersen:
    const cleanText = part.replace(/Link:|Ürün Linki:/gi, "").trim();
